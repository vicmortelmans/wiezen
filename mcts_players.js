...
/**
 * Class managing a whist game from perspective of a single player.
 * This class does not hold state, but it processes state objects.
 */
class Wiezen_mcts {
    ...
    constructor(players, me, game, trump, game_players, card_ids, first_player) {
        ...
        let hand_lookup_string = card_ids.join()
        let other_hands = []
        for (const c of Deck.COLORS) 
            for (const v of Deck.VALUES) 
                if (!hand_lookup_string.includes(c+v)) 
                    other_hands.push(c + v + (c === trump ? '*' : ''))

       let tricks_per_player = {}
       for (player of players)
            tricks_per_player[player = 0]

        // Initialize the state 
        let playing = {
            hand: card_ids,  // the card_ids contain the trump mark (if applicable for the game)
    ...
    play_request(state) {
        ...
        if (state.cards_on_table.length > 0) {
            let playable_cards = []
            let opening_card = state.cards_on_table[0]
            let opening_card_color = opening_card[0]
            if (state.player === this.me) {
                // find card in own hand with same color as opening card
                for (card_id of state.hand) {
                    if (card_id.includes(opening_card_color)) {
                        playable_cards.push(card_id)
                    }
                }
            }
            else {
                // find card in other hands with same color as opening card
                for (card_id of state.other_hands) {
                    if (card_id.includes(opening_card_color)) {
                        playable_cards.push(card_id)
                    }
                }
        ...
        // no card on table or no card with same color found: all cards are playable
        if (state.player === this.me) {
            // all cards in own hand are playable
            playable_cards = [...state.hand]
        }
        else {
            // all cards in other hands are playable
            playable_cards = [...state.other_hands]
        }
        return playable_cards
    }

    /** Advance the given state and return it. */
    play(playing, card_id) {
